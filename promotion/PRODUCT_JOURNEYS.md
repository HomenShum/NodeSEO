...
- **Evidence:** `promotion/evidence/journey-direct-landing.png` — real chromium,
  `1 passed`, exit 0, driven against `examples/site` served locally. The
  Google-origin branch was **not** exercised: it issues a live query against
  Google, which this baseline deliberately did not do.
  Since iteration 1 that screenshot is regenerated by
