...
| 9 | No unexplained console errors or failed requests | UNVERIFIED | Measured 0 console errors and 0 failed requests loading the demo site at 375x812 and 1280x800 in headless chromium — **probe not retained**, same scratchpad capture script as row 4, so nothing in the repo reproduces it. The committed `tests/search-origin.spec.ts` does not close the gap: it collects `failedRequests` (`page.on("requestfailed", ...)`, :42) but **never asserts on them** — its only `expect` is `expect(problems.errors.filter((error) => !isExternalGoogleNoise(error)), problemsSummary(problems)).toEqual([])` (:31), and `failedRequests` is used solely to build that failure message (:56). So the "failed requests" half of this condition has no committed check at all, and the "console errors" half is asserted only after `isExternalGoogleNoise` (:63) drops every error matching `google\|gstatic\|consent\|captcha\|status of 429`. |
...
**Status: NOT PROMOTED** — 2/12 PASS (10, 11), 2 FAIL (2, 5), 8 UNVERIFIED
(1, 3, 4, 6, 7, 8, 9, 12). First recorded as 4/12; rows 4 and 9 were downgraded
on 2026-08-13 because their measurements were real but their probes were not
retained. See PROMOTION_LOG.md, "Correction — 2026-08-13".
